...
try{
	...
	# Initialize log
	Initialize-QlikLog "Remove-QlikGroups"
	Write-QlikLogHeader "Starting the group removal process..." -echo
...
try {
    ...
    Write-QlikLog "$($groups.count) groups imported to remove." -echo
    ...
    $outputFilePath = "$DATA\administer\output\groups_removed.csv"
    ...
    foreach ($group in $groups) {
        ...
        Write-QlikLog "Removing group with name: $name" -echo

        try {
            $groupDetails = Remove-QlikGroup -name $name
...
"@
                ...
                Write-QlikLogError "Group removal encountered an error. $($groupDetails.Error)"
                $group.GroupId = ""
                $group.Error = $groupDetails.Error
            }
        } catch {
            Write-QlikLogError "Encountered an error during the group removal process [$($_.Exception.Message)]"
            $group.groupId = ""
    ...
    Write-QlikLogError "Encountered an unexpected error during the group removal process [$($_.Exception.Message)]"
} finally {
    Write-QlikLogHeader "Group removal process concluded." -echo -character '*'
}