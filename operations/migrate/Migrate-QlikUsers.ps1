...
try{
	...
	# Initialize log
	Initialize-QlikLog "Migrate-QlikUsers"
	Write-QlikLogHeader "Starting the user migration process..." -echo
...
try {
	...
	$appListPath = Join-Path -Path $DATA -ChildPath "migrate-qliksense\input"
    $appList = Get-QlikAppIdList -path $appListPath -file "apps.csv" -property "AppId"
		
    Migrate-QlikAppUsers -app $appList
